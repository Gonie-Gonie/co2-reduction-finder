...
## Actual H5 Files In `.reference`

- The current `.reference/pyCO2module/models/*.h5` files do not match the 25-input/2-output shape described by the Python training path.
- Inspected H5 model configs show 42 models, each with input shape `[None, 24]` and output dimension `1`.
- Total source H5 size: about 101MB.
- Extracted model-weight-only f32 size: about 31.5MB.
